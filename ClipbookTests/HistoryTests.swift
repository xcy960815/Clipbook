...
class HistoryTests: XCTestCase {
  ...
  func testAddingItemThatIsSupersededByExisting() {
    ...
    firstItem.application = "Clipbook.app"
    ...
    secondItem.application = "Clipbook.app"
  ...
  func testAddingItemFromClipbook() {
    let firstContents = [
      HistoryItemContent(
        type: NSPasteboard.PasteboardType.string.rawValue,
        value: "one".data(using: .utf8)
    ...
    let secondContents = [
      ...
      HistoryItemContent(
        type: NSPasteboard.PasteboardType.fromClipbook.rawValue,
        value: "".data(using: .utf8)
      )
    ]
    let second = HistoryItem()
    Storage.shared.context.insert(second)
    second.application = "Clipbook.app"
  ...
  private func temporaryStoreURL(named name: String) -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("clipbook-tests", isDirectory: true)
