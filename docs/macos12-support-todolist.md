...
- 默认数据路径继续保持为 `Application Support/Clipbook/Storage.sqlite`
...
- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook CODE_SIGNING_ALLOWED=NO build` 已通过
...
- `xcodebuild -resolvePackageDependencies -project Clipbook.xcodeproj` 已通过
- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook -configuration Release -derivedDataPath build-local-package -destination 'generic/platform=macOS' ARCHS='x86_64 arm64' ONLY_ACTIVE_ARCH=NO CODE_SIGNING_ALLOWED=NO build` 已通过
...
- 最近一次本地交付中，已将 `build-local-package/Build/Products/Release/Clipbook.app` 移动到 `/Users/opera/Documents/Clipbook.app`

下次需要变更的内容：

- 后续每次本地重新打包并替换 `Documents/Clipbook.app` 时，同步更新本文档中的交付记录
- 如果目标包来自新的构建目录，也一并写明新来源路径

验证结果：

- `/Users/opera/Documents/Clipbook.app` 当前存在

### 2026-04-09 本地构建并替换

本次变更内容：

- 已重新执行本地 `Release` 构建，构建来源路径为 `build-local-package/Build/Products/Release/Clipbook.app`
- 已删除旧的 `/Users/opera/Documents/Clipbook.app`，并用这次新构建的 `.app` 完成替换
- 已对 `/Users/opera/Documents/Clipbook.app` 补做 ad-hoc 签名，避免本地交付包保持未签名状态

下次需要变更的内容：

- 如果继续调样式或功能，下一轮本地构建完成后继续覆盖 `/Users/opera/Documents/Clipbook.app`
...
- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook -configuration Release -derivedDataPath build-local-package -destination 'generic/platform=macOS' ARCHS='x86_64 arm64' ONLY_ACTIVE_ARCH=NO CODE_SIGNING_ALLOWED=NO build` 已通过
- `/Users/opera/Documents/Clipbook.app` 当前存在
- `codesign --verify --deep --strict /Users/opera/Documents/Clipbook.app` 已通过
- `codesign -dv --verbose=4 /Users/opera/Documents/Clipbook.app` 已确认 `Signature=adhoc`
- `lipo -info /Users/opera/Documents/Clipbook.app/Contents/MacOS/Clipbook` 已确认产物包含 `x86_64 arm64`
...
- 已基于这次修改重新本地构建，并再次替换 `/Users/opera/Documents/Clipbook.app`
...
- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook CODE_SIGNING_ALLOWED=NO build` 已通过
- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook -configuration Release -derivedDataPath build-local-package -destination 'generic/platform=macOS' ARCHS='x86_64 arm64' ONLY_ACTIVE_ARCH=NO CODE_SIGNING_ALLOWED=NO build` 已通过
- `/Users/opera/Documents/Clipbook.app` 已完成替换
- `codesign --verify --deep --strict /Users/opera/Documents/Clipbook.app` 已通过
- `lipo -info /Users/opera/Documents/Clipbook.app/Contents/MacOS/Clipbook` 已确认产物包含 `x86_64 arm64`
...
- 在重新替换 `/Users/opera/Documents/Clipbook.app` 前，已先结束运行中的 `Clipbook` 进程，避免替换过程再次被目标目录占用
...
- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook CODE_SIGNING_ALLOWED=NO build` 已通过
- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook -configuration Release -derivedDataPath build-local-package -destination 'generic/platform=macOS' ARCHS='x86_64 arm64' ONLY_ACTIVE_ARCH=NO CODE_SIGNING_ALLOWED=NO build` 已通过
- `killall Clipbook` 已执行，用于替换前结束正在运行的软件
- `/Users/opera/Documents/Clipbook.app` 已完成替换
- `codesign --verify --deep --strict /Users/opera/Documents/Clipbook.app` 已通过
- `lipo -info /Users/opera/Documents/Clipbook.app/Contents/MacOS/Clipbook` 已确认产物包含 `x86_64 arm64`
...
- 将产出最新的 `.app` 文件移动替换更新至 `/Users/opera/Documents/Clipbook.app`，并补充了 `--force --sign -` 签名。

下次需要变更的内容：

- 用户通过真实点击 Clipbook 托盘图标体验面板的尺寸稳定性，验证当列表中包含长图与纯文本混合时，不再出现任何的上下颤抖或缩起回弹的情况。
- 继续关注并推动验证“双击 `Option` 唤醒”的实测以及兼容旧版数据落盘流程。

验证结果：

- `xcodebuild -quiet -project Clipbook.xcodeproj -scheme Clipbook CODE_SIGNING_ALLOWED=NO build` 成功。
...
- 按当前工作区代码重新执行本地 `Release` 构建，构建来源路径为 `build-local-package/Build/Products/Release/Clipbook.app`
- 在替换前先执行 `killall Clipbook`，避免运行中的旧包占用目标目录
- 已删除旧的 `/Users/opera/Documents/Clipbook.app`，并用这次新构建的 `.app` 完成替换
- 已对 `/Users/opera/Documents/Clipbook.app` 重新补做 ad-hoc 签名
...
- 基于当前工作区代码重新执行本地 `Release` 构建，构建来源路径为 `build-local-package/Build/Products/Release/Clipbook.app`
